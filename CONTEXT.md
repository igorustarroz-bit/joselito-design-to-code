...
- **Sección "Variantes y tamaños"**: todas las variantes se muestran juntas, en una sola caja de demo (`<Canvas>`, igual que la sección Demo), en fila con `flex-wrap`, cada una con su título encima — nunca como tabla markdown sola ni como varios `<Story>` sueltos apilados. Se usa el helper `VariantGrid`/`VariantSwatch` de `src/stories/VariantSwatch.jsx` desde el `.stories.jsx` de cada componente (export `AllVariants`/`AllTypes`/`AllSizes`, según convenga), referenciado en el `.mdx` con un único `<Canvas of={Stories.AllX} />`. Los casos concretos añadidos aparte (un estado, un ejemplo con icono, etc.) no se recorren con el grid, pero **siempre** van envueltos en `<Canvas of={...} />`, nunca en `<Story of={...} />` a secas — `<Story>` no incluye la caja de demo (borde/fondo/"Show code") y el elemento queda "suelto" visualmente sobre la página (bug real detectado y corregido en 41 `.mdx` el 2026-07-18). No usar `<Story>` en ningún `.mdx` de este proyecto.
- **Verificación de Storybook**: cualquier cambio en `.mdx`/CSS de documentación que afecte al renderizado se valida visualmente (build estático + screenshot, o Storybook local) antes de darlo por bueno — no basta con leer el código, el bug de `<Story>` vs `<Canvas>` no era detectable así.
